...
cd app && php bin/console cache:clear

# Start Symfony dev server
cd app && symfony serve
```
...
```bash
# Code quality
cd app && vendor/bin/php-cs-fixer fix        # Fix code style (PSR-12)
cd app && vendor/bin/phpstan analyse         # Static analysis (level 8)
cd app && vendor/bin/phpunit                 # Run all tests

# Symfony console
cd app && php bin/console debug:router       # List all routes
cd app && php bin/console debug:container    # List all services
cd app && php bin/console messenger:consume  # Start async worker
```
