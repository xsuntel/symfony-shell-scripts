...
setPhp() {
  ...
  if [ -f "${PROJECT_PATH}"/scripts/console/_symfony_frontend.sh ]; then
    source "${PROJECT_PATH}"/scripts/console/_symfony_frontend.sh
  else
    echo "Please check a file : ./scripts/console/_symfony_frontend.sh" && exit
  fi
  echo
}
